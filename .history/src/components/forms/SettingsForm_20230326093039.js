...
import ColorForm_Settings from "@/components/forms/ColorForm_Settings";
import FontForm from "@/components/forms/FontForm";
...
const SettingsForm = (props) => {
  ...
  const changeHeadingCount = (payload) => {
    console.log("headingCountchanged", payload);
  };
      ...
      <FormWrapper key="frmTextSet" title={props.title}>
        <StyleGrid title="Project Settings">
          <StyleGridItem label="Heading Levels">
            <ThemeDropdown
              key="headingDD"
              onSelect={changeHeadingCount}
              section="project"
              propName="headingCount"
              options={["1", "2", "3", "4", "5", "6"]}
              defaultValue={theme.project.headingCount}
            />
          </StyleGridItem>
        </StyleGrid>
        ...
        <FontForm title="Fonts" />
        <ColorForm_Settings title="Color Palette" />
      </FormWrapper>
    </div>
  );
};
