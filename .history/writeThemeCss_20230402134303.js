...
let levelCount = theme.project.headingCount;
...
function sectionToCSS(themeobj, sectionname, prefix) {
  ...
  keys.forEach((key, i) => {
    if (sectionname == "fonts") {
      ...
      //skip over device specific settings for now
    } else if (isOneOf(key, ["fontSize", "lineHeight"])) {
      //do nothing
    } else {
...
//media queries
for (const device in theme.devices) {
  //skip mobile
  if (device === "mobile") continue; //already handled in root
  ...
  for (const level in LEVELS) {
    ...
    if (level === "body") {
      css += `font-size: ${theme.deviceSettings[device][level].fontSize}px;\n`;
